...
@IBDesignable open class RangeSlider: UIControl {
  ...
  open func updateTrackLayerFrameAndKnobPositions() {
     ...
     */
    open func getLabelText(forValue value: Double, isCheckBool:Bool) -> String {
...
//        guard let labelText = numberFormatter.string(from: NSNumber(value: value)) else { return "" }
        if isCheckBool == true{
            let labelText = String(format: "Min %.0f", value)
            return labelText
        }else {
            let labelText = String(format: "Max %.0f", value)
            return labelText
        }
//        let labelText = String(format: "Min %.0f", value)
//
...
//    upperLabel.string = getLabelText(forValue: upperValue)
    ...
    lowerLabel.string = getLabelText(forValue: lowerValue, isCheckBool: true)
    upperLabel.string = getLabelText(forValue: upperValue,isCheckBool: false)
